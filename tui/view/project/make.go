package project

import (
	"fmt"
	"strings"

	"github.com/archimoebius/hexer/tui/constant"
	"github.com/archimoebius/hexer/util/database"
	tea "github.com/charmbracelet/bubbletea"
	"github.com/charmbracelet/huh"
	"github.com/charmbracelet/lipgloss"
	"github.com/charmbracelet/ssh"
)

type ModelMakeProject struct {
	lg        *lipgloss.Renderer
	styles    *constant.Styles
	form      *huh.Form
	Width     int
	Height    int
	Session   *ssh.Session
	ProjectId string
}

func validateProjectName(m *ModelMakeProject) error {
	s := m.form.GetString("ProjectName")
	if m.form.GetBool("AddProject") && len(s) <= 1 || len(s) > 255 {
		return fmt.Errorf("project name '%s' must be > 1 char and < 255", s)
	}
	return nil
}

func validateProjectDescription(m *ModelMakeProject) error {
	s := m.form.GetString("ProjectDescription")
	if m.form.GetBool("AddProject") && len(s) <= 1 || len(s) > 255 {
		return fmt.Errorf("project description '%s' must be > 1 char and < 255", s)
	}
	return nil
}

func NewModelMakeProject(session *ssh.Session, mpi *constant.MakeProjectInput) (*ModelMakeProject, error) {

	m := &ModelMakeProject{
		Session: session,
	}

	m.Width = mpi.Width
	m.Height = mpi.Height
	m.ProjectId = mpi.ProjectId
	m.lg = lipgloss.DefaultRenderer()
	m.styles = constant.NewStyles(m.lg)

	projectName := ""
	projectDescription := ""

	addOrUpdateProject := "Add Project?"

	if m.ProjectId != "" {
		addOrUpdateProject = "Update Project?"
		project, err := database.GetProjectById(m.ProjectId)

		if err != nil {
			fmt.Printf("failed to load project") // TODO: user error message
		}

		projectName = project.Name
		projectDescription = project.Note
	}

	m.form = huh.NewForm(
		huh.NewGroup(
			huh.NewInput().
				Key("ProjectName").
				Title("Project").
				Placeholder("Project Name").
				Validate(func(s string) error {
					return validateProjectName(m)
				}).
				Value(&projectName).
				Description("The name of the project to track things under"),
			huh.NewInput().
				Key("ProjectDescription").
				Title("Description").
				Placeholder("Short Description").
				Validate(func(s string) error {
					return validateProjectDescription(m)
				}).
				Value(&projectDescription).
				Description("The name of the project to track things under"),
			huh.NewConfirm().
				Key("AddProject").
				Title(addOrUpdateProject).
				Affirmative("Yes").
				Negative("No").
				Validate(func(c bool) error {
					if c { // all this crap simply b/c m.form.GetBool("AddProject") value isn't updated -> followed by a final form validate check... pita...
						err := validateProjectName(m)
						if err != nil {
							return err
						}

						err = validateProjectDescription(m)
						if err != nil {
							return err
						}

					}
					return nil
				}),
		),
	).
		WithWidth(m.Width / 2).
		WithShowHelp(false).
		WithShowErrors(false)

	return m, nil
}

func (m ModelMakeProject) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmds []tea.Cmd

	switch msg := msg.(type) {
	case tea.WindowSizeMsg:
		m.Width = msg.Width
		m.Height = msg.Height
	case tea.KeyMsg:
		switch msg.String() {
		case "esc":
			return m, constant.SwitchModeCmd(constant.ModeListProject,
				constant.NewListProjectInput(m.Width, m.Height),
			)
		}
	}

	form, cmd := m.form.Update(msg)
	if f, ok := form.(*huh.Form); ok {
		m.form = f
		cmds = append(cmds, cmd)
	}

	if m.form.State == huh.StateCompleted {

		if m.form.GetBool("AddProject") {
			projectName := m.form.GetString("ProjectName")
			ProjectDescription := m.form.GetString("ProjectDescription")

			if m.ProjectId != "" {
				err := database.UpdateProject(m.ProjectId, projectName, ProjectDescription)

				if err != nil {
					return m, constant.ErrorMessage(err)
				}
			} else {

				err := database.AddProject(projectName, ProjectDescription)

				if err != nil {
					return m, constant.ErrorMessage(err)
				}
			}
		}

		return m, constant.SwitchModeCmd(constant.ModeListProject,
			constant.NewListProjectInput(m.Width, m.Height),
		)
	}

	return m, tea.Batch(cmds...)
}

func (m ModelMakeProject) Init() tea.Cmd {
	return m.form.Init()
}

func (m ModelMakeProject) View() string {
	s := m.styles
	header := constant.AppBoundaryView(m.styles, m.Width, "", "")

	v := strings.TrimSuffix(m.form.View(), "\n\n")
	form := m.lg.NewStyle().Margin(1, 0).Render(v)

	errors := m.form.Errors()

	if len(errors) > 0 {
		header = constant.AppErrorBoundaryView(m.styles, m.Width, m.errorView())
	}
	body := lipgloss.JoinHorizontal(lipgloss.Top, form)

	footer := constant.AppBoundaryView(m.styles, m.Width, m.form.Help().ShortHelpView(m.form.KeyBinds()), "")
	if len(errors) > 0 {
		footer = constant.AppErrorBoundaryView(m.styles, m.Width, "")
	}
	m.styles.Base.Width(m.Width).Height(m.Height)

	return s.Base.Render(header + "\n" + body + "\n\n" + footer)
}

func (m ModelMakeProject) errorView() string {
	var s string
	for _, err := range m.form.Errors() {
		s += err.Error()
	}
	return s
}
